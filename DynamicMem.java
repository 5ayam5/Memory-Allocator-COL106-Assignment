...
public abstract class DynamicMem {
    ...
    // This function should allocate a contiguous array of size blockSize and return
    // the first address.
    // It should return -1 if memory is not avaiable.
    // The free blocks list and the allocated blocks list should be suitably
    // modified
    // Algorithm:
    // 1. Search in the free block dictionary to find a block of size >= blockSize
    // using the find function
    // 2. If found, check if splitting of the block is needed.
    // 3. If yes, split the block and insert the two blocks into the free and
    // allocated blocks dictionary
    // 3.1. Delete the block from the free block dictionary
    // 4 If no, insert the block into allocated blocks dictionary and remove it from
    // free blocks dictionary
    ...
    // Algorithm:
    // 1. Create a new BST/AVL Tree indexed by address. Use AVL/BST depending on the
    // type.
